...
- **Architecture-bound leaf skill** — a router alone does not isolate the two architectures, because a leaf description can match trigger words on its own and the two families give contradictory advice. Every leaf that serves one `Architecture:` value must say `Applies only in a <architecture> profile app.` and open its boundary with `WHEN NOT: A <other> profile app — use <sibling>.` Add the skill name to `LAYERED_ONLY_SKILLS` or `RICH_MODELS_ONLY_SKILLS` in `scripts/verify_plugins.sh`, which fails until both clauses are present. The same contract covers the CSS axis through `TAILWIND_ONLY_SKILLS` and `PLAIN_CSS_ONLY_SKILLS`; skills selected by Authorization, Runtime, Database, Tenancy, or Deployment are still out of scope, and the header records the specific exclusions. `check_reachability` separately fails the gate when a skill is named by no router and no reachable skill and is not `user-invocable`, so a new leaf must be routed from somewhere or declared user-invocable.

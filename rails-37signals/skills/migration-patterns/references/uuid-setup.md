...
> Fizzy profile only: this entire reference. UUIDv7 primary keys, deterministic fixture ordering,
> `id` tie-breakers on every ordering scope, and account context are **one non-negotiable package** —
> adopt them together or not at all. An ONCE-compatible application uses integer ids. UUIDv7 is also
> an edge-Rails feature; verify it exists in your Rails release. Read alongside
> `../../testing-patterns/references/fixture-patterns.md` and
> [`09-data-search.md`](../../../docs/37signals-playbook/09-data-search.md).

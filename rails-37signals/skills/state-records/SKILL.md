...
**Tech Stack:** Rails 8.1+, ActiveRecord associations
**Pattern:** One state model per boolean you'd normally add
**Naming:** Noun forms (Closure, Publication, Goldness, NotNow, Archival)

A state record is also **a resource**: `resource :closure` gives you `POST` to close and `DELETE` to
reopen, with no custom controller action. The naming and the routing are the same decision — see
`crud-patterns`.

Verb methods on the parent stay idempotent and transactional, and emit their event inside the same
transaction.

> Fizzy profile only: the `:uuid` types and `account:references` in the generator commands below.
...
- **Always:** Create a state record for business-meaningful states, track who and when, use
  `where.missing` for negative scopes, add a **unique** index on `parent_id` so `has_one` is a
  database guarantee, touch the parent record, expose the state as a singular resource, keep the
  transition idempotent and transactional, write tests for state transitions
- **Ask first:** Before using a boolean column for business state, before adding complex metadata
  (it might deserve its own model)
- **Never:** Use booleans for important business state, skip who/when tracking, allow multiple state
  records per parent, add a custom controller verb for a transition, skip event tracking for state
  changes

See [`03-models.md`](../../docs/37signals-playbook/03-models.md) and
[`04-controllers-routing.md`](../../docs/37signals-playbook/04-controllers-routing.md).

...
**Find the nearest existing example in this codebase and copy its shape.** Nearly every kind of thing
you need to write already exists somewhere. This is the single most load-bearing instruction here —
consistency beats individual judgement.

Then read the `## Application profile` block in `CLAUDE.md`. Several conventions below are
Fizzy-profile only (account scoping, UUID ids, `params.expect`). In an ONCE-compatible application,
applying them is a defect, not thoroughness. If no profile is recorded, ask before starting.
...
| event-tracking | Domain events, activity feeds, webhooks, the SSRF boundary |
| css-design | Plain CSS, oklch token layering, dark mode, a11y baseline |
| data-search | In-database full-text search |
| content-storage | Action Text, Active Storage authorization, export/import |
| tooling-ci-deploy | `bin/setup`, `config/ci.rb`, Docker, deploy, operability |

For skill selection guidance, see @../skills/implementation-workflow/references/delegation-guide.md
...
For 10 detailed workflow patterns, see @../skills/implementation-workflow/references/workflow-patterns.md
...
**Multi-tenant consistency (Fizzy profile only):** Every new table gets `account_id`. Every query
scopes through `Current.account`. Every controller finds resources via `Current.account.resources`.
Skip all of this in an ONCE-compatible application — it has no account layer.

**Authorization is scoping:** a record the current actor cannot reach is not found, and `find` raises
→ 404. Do not add an authorization call. Privilege checks beyond visibility are `can_<verb>_<noun>?`
predicates on `User`.
...
**Rich models over services:** Business logic lives in models. Controllers call model methods. No
`app/services/` directory. A plain object is fine when it is the clearest domain object — put it in
`app/models/` with a domain name, not in a category directory.

**Concerns are namespaced under the model:** `app/models/card/closeable.rb` defining `Card::Closeable`.
Promote to `app/models/concerns/` only when a second model includes it.

**Serialize job context:** a job never inherits ambient `Current` across the queue boundary. Capture
what it needs at enqueue.
...
- **Does it need to exist?** Reuse a local pattern, a Rails API, the stdlib, or a browser feature
  before adding anything — including a new dependency.
- **New resource vs. existing?** If it has its own lifecycle, create a new resource.
- **Namespaced concern vs. shared concern?** Namespaced under the model by default; move it to
  `app/models/concerns/` at the second includer, not before.
...
- **Validation vs. database constraint?** An absolute invariant gets a unique index or constraint,
  not only a validation.
- **New directory?** If you are creating one to hold a *category* of object, you have built a layer.
  Stop.

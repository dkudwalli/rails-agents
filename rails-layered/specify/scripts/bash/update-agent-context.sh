...
create_new_agent_file() {
    ...
    # Replace template placeholders. This kit ships for Rails only, so the structure, commands and
    # style line are the pack's own conventions rather than a per-language lookup.
    local project_structure="app/\\nlib/\\nspec/"
    local commands="bundle exec rspec   # tests\\nbundle exec rubocop   # style"
    local language_conventions="Ruby: follow .rubocop.yml; see the layered-conventions skill for per-layer rules"

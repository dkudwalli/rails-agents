...
See [policy-patterns.md](../skills/policy-patterns/references/policy-patterns.md).

## Controller Authorization

Every action must call `authorize` (single record) or `policy_scope` (collection), and attribute
updates go through `permitted_attributes`. `ApplicationController` rescues
`Pundit::NotAuthorizedError` so an unauthorized action flashes and redirects rather than 500s. See
[testing-and-controllers.md](../skills/policy-patterns/references/testing-and-controllers.md).

## Testing

ALWAYS write policy specs. Required contexts: unauthenticated (`nil`), regular user, owner, admin,
custom actions — using `forbid_action` / `permit_actions` against a `described_class.new(user, record)`
subject. See
[testing-and-controllers.md](../skills/policy-patterns/references/testing-and-controllers.md) for
complete examples.
...
## References
- [policy-patterns.md](../skills/policy-patterns/references/policy-patterns.md) -- ApplicationPolicy base + 5 policy patterns
- [testing-and-controllers.md](../skills/policy-patterns/references/testing-and-controllers.md) -- RSpec tests, controller integration, view checks

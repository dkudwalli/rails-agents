...
`rails-layered` plugin to pick up new scripts and templates.
...
2. Resolve the plugin's own directory. **You know the absolute path you loaded this `SKILL.md`
   ...
   ```bash
   PLUGIN_ROOT="<absolute path to the rails-layered plugin root>"; ls "$PLUGIN_ROOT/specify"
   ```
...
3. Copy the seed. **The idempotency differs per subtree — this is the whole reason this is a skill
   ...
   ```bash
   PLUGIN_ROOT="<absolute path to the rails-layered plugin root>"; \

...
- **God Model**: A model keeps only persistence, validations, associations, and simple scopes. When it grows past that, extract business logic to services and complex queries to query objects. The `extraction-timing` skill owns the size thresholds and the churn x complexity analysis for models that are already large.

...
Every query spec must prove the query cannot see another account's rows. Use the shared example
rather than restating the two-account setup in each file:

```ruby
# spec/support/shared_examples/tenant_isolation.rb
RSpec.shared_examples 'tenant isolated' do
  it 'excludes other tenant data' do
    other_record = create(factory, account: create(:account))

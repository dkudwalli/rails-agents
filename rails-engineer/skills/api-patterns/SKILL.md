...
- **Jbuilder templates live in the same view directory as the HTML templates.** No serializer
  classes, no `as_json` overrides on models, no `app/serializers/` directory
- HTTP status codes for success/errors
- Token-based authentication for API (not OAuth unless required)

**Record how the request authenticated, and gate capabilities on the credential type.** A bearer
token should work on JSON requests only; CSRF and bot-access policy follow from the credential, not
from a blanket `skip_before_action`. Scoped API tokens are about ten lines — see `auth-setup`.

**Do not make arbitrary internal JSON a public contract.** Render the payload explicitly and
whitelist what it contains; a jbuilder template you can read is the contract.
...
> Fizzy profile only: the account scoping in these examples
> (`/accounts/:account_id/...`, tokens scoped to an account).
...
- Add serializer classes or override `as_json` on a model (use jbuilder templates)
...
- Let a bearer token authenticate an HTML request

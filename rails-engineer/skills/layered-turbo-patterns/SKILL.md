...
description: >-
  Implements Turbo Drive, Turbo Frames, Turbo Streams, and broadcasts for fast page updates with minimal JavaScript. Use when adding partial page updates, live updates, inline editing, morphing, or prefetch, or when the user mentions Turbo, frames, or streams. WHEN NOT: Complex client-side behaviour needing a Stimulus controller (see stimulus-patterns), API-only JSON endpoints (see api-versioning), or static pages.
license: MIT

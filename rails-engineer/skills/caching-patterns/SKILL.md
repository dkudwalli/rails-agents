...
**Use Rails' cache layers before inventing one.** Do not hide cache keys behind a home-grown wrapper —
the key should be readable at the call site. Choose the cache store at the environment boundary, not
in application code. Version the ETag inputs when a rendering contract changes, and add **every**
non-record input to a fragment's cache key.

**Name the query shapes you cache.** Ordering and preload scopes belong on the model; do not
speculatively preload everything. Give unbounded maintenance work `find_each` or a resumable job
rather than a bigger cache.

## Project Knowledge

**Stack:** Turbo for page refreshes, ETags with conditional GET, fragment caching in ERB views,
collection caching for lists.

> Profile decision: the cache store is **Solid Cache** (Fizzy, database-backed) or **Redis** (ONCE
> apps). It is one decision together with the queue and cable backends — never mix the two families.

> Fizzy profile only: account-scoped cache keys and `app.myapp.com/123/projects/456` URLs.
...
### Never
- Run two cache backends, or mix the Solid and Redis families
- Wrap cache keys in a home-grown abstraction
- Cache without considering invalidation strategy
- Forget `touch: true` with Russian doll caching
- Omit a non-record input from a fragment's cache key
...
- Cache publicly when the attached record is private
- Cache across account boundaries in multi-tenant apps

See [`20-performance-operability.md`](../../docs/37signals-playbook/20-performance-operability.md).

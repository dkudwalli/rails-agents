...
description: >-
  ...
  description. Use when starting full-ceremony SDD work on a new feature, after
  sdd-constitution exists. WHEN NOT: A small bug fix or small feature — use
  sdd-change-specify; refining an existing spec's open questions — use sdd-clarify.
---
...
The text the user typed when invoking this skill **is** the feature description. Assume you always have it available in this conversation. Do not ask the user to repeat it unless they invoked the skill with nothing after it.
...
2. **Create the feature branch** by running the script with `--short-name` (and `--json`). In sequential mode, do NOT pass `--number` — the script auto-detects the next available number. In timestamp mode, the script generates a `YYYYMMDD-HHMMSS` prefix automatically:
   ...
   **Branch numbering mode**: Before running the script, check if `.specify/init-options.json` exists and read the `branch_numbering` value.
   ...
   - Example: `.specify/scripts/bash/create-new-feature.sh "Add user authentication" --json --short-name "user-auth"`
   - Timestamp: `.specify/scripts/bash/create-new-feature.sh "Add user authentication" --json --timestamp --short-name "user-auth"`

   Pass the description **once**, as the only positional argument. The script joins every positional
   into one description, so repeating it produces a doubled string.

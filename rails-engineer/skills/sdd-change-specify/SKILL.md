---
name: sdd-change-specify
description: >-
  Create a lightweight change specification for bug fixes and small features —
  skips the full SDD ceremony. Use when Workflow: sdd is selected but the change is
  small enough to skip constitution/clarify/spec-review ceremony. WHEN NOT: A large
  or ambiguous feature needing full ceremony — use sdd-specify.
---
...
2. **Create the feature branch** by running the script:
   ...
   **Branch numbering mode**: Check if `.specify/init-options.json` exists and read the `branch_numbering` value.
   ...
   Run: `.specify/scripts/bash/create-new-feature.sh "<feature description>" --json --short-name "<name>"`

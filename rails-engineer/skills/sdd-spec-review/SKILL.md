...
description: >-
  Adversarial review of the feature spec from security, performance, edge-case,
  scalability, and regulatory perspectives to catch blind spots before planning.
  Use after sdd-specify (and sdd-clarify, if used) to stress-test the spec before
  planning. WHEN NOT: The spec has open questions from the author's own
  uncertainty — use sdd-clarify first; already past spec review — use sdd-plan.
---
...
Context for review focus: the user's request from the invoking message

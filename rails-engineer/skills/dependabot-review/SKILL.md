...
Resolve the current repo before anything else:

```bash
gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null || echo "(unknown — run from inside the repo)"
```
...
Read the `behavioral-guidelines` skill § Reporting Rules first — especially the ban on vague
recommendations. "Review before merging" without naming what to review is not a finding.

...
- **Self-contained:** it owns its association, scopes, predicates, and verbs — a vertical slice, not a
  bag of helper methods
- **Cohesive:** focused on one aspect (`Closeable`, `Watchable`, `Searchable`)
- **Composable:** models include several concerns to build up behavior
- **Adjective-named:** concerns are adjectives; the state records they create are nouns

The class body of a mature model reads as a table of contents — a list of `include`s, not a wall of
methods. A 95-line model including 24 concerns is the intended shape.

## Where a concern lives

**Default: namespaced under the model it extends.** `app/models/card/closeable.rb` defining
`Card::Closeable`.

**Promote to `app/models/concerns/` only when a second model includes it.** The flat directory is
reserved for the genuinely shared — in the source applications it holds a handful of files. This is
what keeps `Card`'s twenty-odd features from colliding with `Comment`'s in one namespace: both can
have a `Searchable`, and they are different files.

## When to extract a concern

**A single model is reason enough.** Splitting one growing model into namespaced concerns is the
primary use, not a fallback — you do not need a second includer to justify `Card::Closeable`.

### Extract when you see:

1. **A cohesive slice of one model's behaviour**
   ```ruby
   # close/reopen, its closure association, its scopes and predicates
   # Extract to: app/models/card/closeable.rb -> Card::Closeable
   ```

2. **Repeated associations across models** (now the flat directory earns its place)
   ```ruby
...
3. **Repeated scopes**
   ...
   # Extract to: app/models/concerns/timestampable.rb
   ```

4. **Repeated controller patterns**
   ```ruby
   # Multiple controllers load the same parent resource
   # Extract to: app/controllers/concerns/card_scoped.rb
   ```

### Do NOT extract when:
- You'd create a god concern with unrelated methods
- The behaviour is one method that belongs on the model directly
- You are reaching for a concern because the logic feels like a service — put it in the model

A shared concern that requires methods from its includers should list them in a comment at the bottom.
...
- **File path follows the includer count**: `app/models/card/closeable.rb` (`Card::Closeable`) for one
  model; `app/models/concerns/assignable.rb` (`Assignable`) once two include it

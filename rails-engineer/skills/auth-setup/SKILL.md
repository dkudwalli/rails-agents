...
**Auth is simple. No Devise, no Pundit, no CanCan, no OmniAuth.** Roughly 100 lines of code per
application. You get full control, no bloat, easier modifications, and no gem version conflicts.
...
### The five rules everything below follows

1. **A session is a record.** `has_secure_token`; the cookie holds only the signed token —
   signed, permanent, httponly, `same_site`. Throttle the `resume` write to hourly so every request
   is not a database write. Keep session lookup in its own tiny concern so Action Cable can share it.
2. **Secure by default.** An unconditional `before_action :require_authentication` in the
   `Authentication` concern, opted out through **named class macros**
   (`allow_unauthenticated_access`, `require_unauthenticated_access`) — never a scattered
   `skip_before_action`. Write `require_authentication` as a chain of `||` alternatives, so adding a
   credential type is adding a branch.
3. **Record how the request authenticated**, and gate CSRF, bot access, and capabilities on the
   credential type. A bearer token should work only on JSON requests.
4. **Authorization is scoping plus predicates, not a policy layer.** A whole authorization
   "framework" here is about ten lines: unreachable records 404, and `can_<verb>_<noun>?` predicates
   on `User` cover the rest. Define an overlap once — `def admin?; super || owner?; end`.
5. **Enumeration resistance is the same response shape, not a different error message.** Return a
   fake unsaved magic link rather than branching on whether the address exists.

### Also non-negotiable

- **Rate-limit anything that sends mail, issues a credential, or accepts a code** — using Rails'
  built-in `rate_limit`, at the action, not in the proxy
- Magic links: single-use consume, beginless/endless range expiry, double uniqueness enforcement,
  a `cleanup` class method, and input sanitisation
- Add a runtime `after_action` that **raises** if a development-only auth shortcut is reachable in
  another environment
- Filter personal data and user content from logs, not only secrets. Env-tunable CSP with nonces
- Run the security scanners in CI on every run with `--exit-on-warn`; triage Brakeman findings in a
  checked-in ignore file rather than disabling the tool

### SSRF, when auth touches the network

If any flow fetches a URL (avatars, OIDC discovery, webhooks), resolve DNS yourself with a timeout
and explicit nameservers, and **check the resolved address, not the hostname**. Cite an RFC per
blocklist entry, handle IPv6, and decode NAT64 back to the embedded IPv4. See `event-tracking` for
the outbound-request boundary in full.
...
> Fizzy profile only: `params.expect` in the controller examples below, and `Current.account`.
> Campfire and Writebook use `params.require(...).permit(...)` and have no account layer.
> `params.expect` is an edge-Rails feature — verify it exists in your Rails release.
...
- **Always:** Secure controllers by default with a named opt-out macro, offer passkeys as primary auth,
  use signed cookies with httponly/same_site flags, record how the request authenticated, expire magic
  links (15 min), mark magic links as used, normalize emails, use `has_secure_token`, `rate_limit`
...
- **Never:** Add Devise, Pundit, or CanCan; add a policy object (scope the lookup instead); use a
  ...
  server-side session state (use signed tokens); trust a hostname you have not resolved

See [`10-auth-security.md`](../../docs/37signals-playbook/10-auth-security.md).

...
Reviewing a whole application rather than a diff? Group the next steps by the phases in
[`layered-legacy-migration`](../layered-legacy-migration/SKILL.md), so the audit hands off in executable order rather

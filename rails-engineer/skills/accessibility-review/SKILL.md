...
Read the `behavioral-guidelines` skill § Reporting Rules before writing the report. The
checklist above is a review aid, not an output template — report the failures with `file:line` and the fix,
and leave out the criteria that passed.

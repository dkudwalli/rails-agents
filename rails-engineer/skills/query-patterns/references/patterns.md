...
Scope through the association, never by a `where(account_id:)` column match — the association is the
only form that cannot be reached with a forged id.
...
Queries return relations, so callers can keep chaining:

```ruby
events = Entities::SearchQuery.new(account: account).call
upcoming = events.where("event_date > ?", Date.current)
paginated = upcoming.page(params[:page]).per(20)
```

A query that needs several others composes them rather than reimplementing their clauses:

```ruby
# app/queries/reports/complex_report_query.rb
module Reports
  class ComplexReportQuery < ApplicationQuery
    def initialize(account:)
      @events_query = Entities::SearchQuery.new(account: account)
      @revenue_query = Orders::RevenueReportQuery.new(account: account)
    end
...
```ruby
# Eager loading — pick the strategy per association
account.events
  .includes(:venue)              # belongs-to
  .includes(attendees: :user)    # nested
  .preload(:documents)           # separate query, no JOIN
  .eager_load(:primary_contact)  # LEFT JOIN, filterable in SQL

# Batch processing — never load an unbounded set into memory
account.events.find_each(batch_size: 100) { |event| yield event }

# Subquery instead of pluck — keeps the ids in the database
active_vendor_ids = account.vendors.active.select(:id)
account.events.where(vendor_id: active_vendor_ids).order(created_at: :desc)
```

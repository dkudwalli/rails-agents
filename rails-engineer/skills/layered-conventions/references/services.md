...
- Return a Result object (`Data.define(:success, :data, :error, :code)` with `success?`/`failure?` predicates); `code` is the typed error symbol callers branch on, `nil` on success

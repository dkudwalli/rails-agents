...
Every service below inherits from this. It supplies the `success`/`failure` constructors and the
class-level `.call` shortcut, so subclasses only implement `#call`.
...
class ApplicationService
  ...
  def success(data = nil) = Result.new(success: true, data: data, error: nil, code: nil)

  def failure(error, code = :unknown, data: nil)
    Result.new(success: false, data: data, error: error, code: code)
  end
end
```

Typed error codes and how controllers branch on them live in
[`layered-rails-architecture/references/error-handling.md`](../../layered-rails-architecture/references/error-handling.md).

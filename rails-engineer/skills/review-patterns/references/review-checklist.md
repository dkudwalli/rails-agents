...
- [ ] Uses the queue backend the app's profile selected — Solid Queue (Fizzy) or Resque (ONCE) — and
      only one of them. Sidekiq is used in neither
...
- [ ] Enqueued from `after_*_commit`, never `after_save`
- [ ] Serializes the context it needs rather than relying on ambient `Current`

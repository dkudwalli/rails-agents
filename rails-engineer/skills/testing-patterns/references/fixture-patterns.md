...
**Two things must hold together** when a project adopts UUID primary keys with fixtures:

1. Fixture ids must **sort like integer ids would**, so a scope ordered by `created_at, id` produces
   the same result every run. UUIDv7 is time-ordered, which is why it is the chosen variant
2. Every fixture timestamp must be in the past, so "recent" and "expired" scopes behave the same on
   every machine and every day

Get either wrong and the suite fails intermittently in a way that looks like a scope bug.

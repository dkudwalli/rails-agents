...
Services return Result objects instead of raising exceptions. One definition for the whole app,
living on `ApplicationService` — see the `service-patterns` skill for the full base class:
...
Everything below assumes that shape: `code` is the typed symbol, `error` the user-facing message.
...
class ApplicationService
  ...
  # The one `failure` constructor, logging every failure with context on the way out
  def failure(error, code = :unknown, data: nil, exception: nil)
    log_error(code, error, exception)
    Result.new(success: false, data: data, error: error, code: code)
  end
...
```ruby
# With Sentry/Rollbar — same `failure` constructor, one more side effect
def failure(error, code = :unknown, data: nil, exception: nil)
  if exception && should_report?(code)
    Sentry.capture_exception(exception, extra: { code: code, message: error })
  end
